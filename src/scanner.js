...
module.exports = function ({ config, db, logger }) {
  Observable
    .create(o => {
	  logger.info('****************************Observed Creation***************************')
	  if (typeof config.scanner.paths == 'string'){
		  logger.info('****************************' + config.scanner.paths + '***************************')
	  }else{
		  logger.info('**************************** Paths Are No String ***************************')
	  }
  ...
  async function cleanDeleted () {
    ...
    while (true) {
	  ...
	  //CHANGED START
	  logger.info('****************************PROMISE ALL ROWS***************************')
      await Promise.all(rows.map(async ({ doc }) => {
        try {
		  console.log('****************************START***************************')
		  let idx = -1
		  const mediaPath = path.normalize(doc.mediaPath)
		  if(config.scanner.paths instanceof Array){
			  for(let i=0; i<config.scanner.paths.length; i++){
				  const mediaFolder = path.normalize(config.scanner.paths[i])
